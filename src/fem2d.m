...
max_error = 0;
for ii = 1:T
    vertice_numbers = t(ii, :);
    x = p(vertice_numbers, 1);
    y = p(vertice_numbers, 2);
    z = U(vertice_numbers);
    %disp(x);disp(y); disp(z); 
    actual_value = (1 - x.^2)/2;
    %disp(actual_value);
    for jj = 1:2:41
        tail = (sinh(jj*pi*(1 + y)/2) + sinh(jj*pi*(1-y)/2));
        middle = sin(jj*pi*(1 + x)/2)/(jj^3 * sinh(jj*pi));
       actual_value = actual_value - ((16/ (pi^3))* middle .* tail);  
    end
    error = log(abs(max(actual_value - z)));
